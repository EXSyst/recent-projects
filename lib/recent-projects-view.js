...
module.exports = RecentProjectsView = (function(superClass) {
            ...
            return function(err, data) {
                if (err != null) {
                    ...
                    return data.forEach(function(arg1) {
                        var lastOpened, tileUri, uri;
                        uri = arg1.uri, lastOpened = arg1.lastOpened;
                        tileUri = path.join(uri, ".project-tile.png");
                        return fs.exists(tileUri, function(exist) {
                            if (exist) {
                                return _this.setDetails(uri, {
                                    tile: tileUri
                                });
                            }
                        });
    ...
    RecentProjectsView.prototype.setList = function(projects) {
        ...
        return projects.forEach((function(_this) {
            return function(project) {
                var branch, entry, lastOpened, repo, uri;
                uri = project.uri, lastOpened = project.lastOpened;
                if (uri !== atom.project.path) {
                    try {
                        ...
                        return this.li({
                            ...
                            return function() {
                                _this.div({
                                    ...
                                    }, relativeToHomeDirectory(path.dirname(uri)));
                                    _this.br();
                                    if (project.devMode) {
                                        _this.div({
                    ...
                    entry.on('click', _this.openProject.bind(_this, [uri], entry));
                    entry.find('.project-delete').on('click', function(ev) {
                        ev.stopPropagation();
                        return _this.removeProject(uri, entry);
                    });
    ...
    RecentProjectsView.prototype.setDetails = function(uri, data) {
        var entry;
        entry = this.projectList.children().filter(function() {
            return this.getAttribute('data-uri') === uri;
        });
        if (entry.length) {
            if ('tile' in data) {
                if (data.tile) {
    ...
    RecentProjectsView.prototype.removeProject = function(uri, entry) {
        if (RecentProjects == null) {
            RecentProjects = require('./recent-projects');
        }
        return RecentProjects.remove(uri, (function(_this) {
            return function(err) {
                if (err == null) {
                    return entry.remove();
                }
    ...
    RecentProjectsView.prototype.createNewFile = function() {
        ...
        return atom.workspaceView.trigger('application:new-file');
    };

    RecentProjectsView.prototype.closeAfterOpenProject = function() {
        var showTree;
        if ((atom.project.path != null) || atom.workspaceView.getActivePaneView().getItems().length > 1 || parseFloat(atom.getVersion()) >= 0.124) {
            atom.workspace.getActivePane().removeItem(this);
            showTree = function() {
                return atom.workspaceView.trigger("tree-view:show");
            };
