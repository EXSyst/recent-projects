...
module.exports = RecentProjectsView = (function(superClass) {
    ...
    RecentProjectsView.prototype.handleEvents = function() {
        ...
        atom.commands.add(this.element, {
            'recent-projects:move-left': this.moveLeft.bind(_this),
            'recent-projects:move-right': this.moveRight.bind(_this),
            'recent-projects:open-selected': this.openSelected.bind(_this),
            'recent-projects:remove-selected': this.removeSelected.bind(_this),
        });
    }
    ...
    RecentProjectsView.prototype.moveLeft = function() {
        if (this.selectedEntry != null) {
            ...
            var prev = selectedEntry.prev('.project-entry')[0];
            if (prev != null) {
                this.selectEntry(prev);
            } else {
                this.selectEntry(this.projectList.find('.project-entry').last()[0]);
            }
        } else {
            this.selectEntry((ref = this.projectList.find('.project-entry').last()) != null ? ref[0] : null);
        }
    };
    ...
    RecentProjectsView.prototype.moveRight = function() {
        if (this.selectedEntry != null) {
            var selectedEntry = $(this.selectedEntry);
            var next = selectedEntry.next('.project-entry')[0];
            if (next != null) {
                this.selectEntry(next);
            } else {
                this.selectEntry(this.projectList.find('.project-entry').first()[0]);
            }
        } else {
            this.selectEntry((ref = this.projectList.find('.project-entry').first()) != null ? ref[0] : null);
        }
    };
    ...
    RecentProjectsView.prototype.removeSelected = function() {
        ...
        $(this.selectedEntry).find('.project-delete')[0].click();
    };
