...
Project.prototype.getBranch = function() {
    var branch = null;
    try {
        var repo = new GitRepository(project.getPaths()[0]);
...
Project.prototype.getFtpUrl = function() {
    ...
    try {
        ...
        if (config.remote) {
            ftpUrl = path.join(ftpUrl, config.remote);
        }
...
Project.prototype.getTile = function() {
    ...
    this.getPaths().forEach(function(path) {
        if (tileUri)
            return;
        var uri = path.join(arg2, ".project-tile.png");
        return fs.exists(uri, function(exist) {
            if (exist) {
                var tileUri = uri;
            }
        });
